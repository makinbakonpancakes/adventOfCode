"""
Advent of Codemakin_bakon_pancakes 4*
...
for move in input:
    ...
    if move == '>':
        ...
        if location[0] >= cols:
            ...
            santaMap[location[1]][location[0]] += 1

    elif move == '^':
        if location[1] == 0:
            top = [0] * cols
            santaMap.insert(0, top)
            santaMap[0][location[0]] = 1
        else:
            location[1] -= 1
            santaMap[location[1]][location[0]] += 1
        ...
        if location[1] >= rows:
            bottm = [0] * cols
            santaMap.append(bottm)
            santaMap[len(santaMap) - 1][location[0]] = 1
        else:
            santaMap[location[1]][location[0]] += 1

presents = 0
for row in santaMap:
    presents += len(row) - row.count(0)
print(presents)